...
project_pollock <- function(h1,h2,R,x0,y0,theta,n,alpha){
	...
	n_shad <- ceiling((2.0*R)/0.25) + 1
	dR <- (R*2.0)/(n_shad-1)
	
	xarr <- seq(-R,R,dR)
	yarr <- h1*tan(theta) - sqrt(-xarr^2+R^2)

	xyarr <- transform_xy(cbind(xarr,yarr),c(x0,y0),alpha-3*pi/2)
	...
	zp <- as.numeric(tmp$ut)*h2
	#rp <- as.numeric(tmp$vt)*R
	rp <- as.numeric(tmp$vt)*sqrt(R*R-xarr*xarr)
...
pollock_shadow <- function(h1,h2,R,x0,y0,theta,alpha, n, n_sectors){
	...
	phi_arr <- seq(pi,2*pi,dphi)

	xarr <- R*cos(phi_arr)
	yarr <- R*sin(phi_arr) + h1*tan(theta)

	xyarr <- transform_xy(cbind(xarr,yarr),c(x0,y0),alpha-3*pi/2)

	xy_shad <- rbind(xy_shad,xyarr)

	phi_arr <- seq(0,pi,dphi)
	xarr <- R*cos(phi_arr)
	yarr <- R*sin(phi_arr)
...
eval_shad_v5 <- function(params,xy0,R,theta_sat,alpha_sat,theta_sun,alpha_sun){
	...
	tmp <- project_pollock_quantitativeC(h1,h2,R,xy0[1],xy0[2],theta_sat,n)
	xy1 <- xy0 + tmp*c(cos(alpha_sat),sin(alpha_sat))

	xy_shad_pix <- xy[shad,,drop=FALSE]
	xy_shad_pix[,1] <- xy_shad_pix[,1] - xy1[1]
	xy_shad_pix[,2] <- xy_shad_pix[,2] - xy1[2]
	...
	phi_arr <- phi_arr - (alpha_sun - 3*pi/2)
	...
	if(Debug){
		windows()
		plot(x,y, xlim=1.2*c(-R,R),ylim=c(-5,15),asp=1,pch=16)
		abline(h=0,lty=2)
		abline(v=0,lty=2)
	}

	shad_pol <- project_pollockC(h1,h2,R,0,0,theta_sun,3*pi/2,n,0.25)
	#shad_polR <- project_pollock(h1,h2,R,0,0,theta_sun,n,3*pi/2)

	if(Debug){
		blob <- Blobs[id,]
		blob$x <- 0
		blob$y <- 0
		display_all_blobs(blob,"blue")
		lines(shad_pol,col="green")
		#lines(shad_polR[,],col="red",lwd=2)
	}
	...
	for(i in 1:(length(xarr)-1)){
		x1 <- xarr[i]
		x2 <- xarr[i+1]
		
		y1 <- (shad_pol[i,2] + shad_pol[i+1,2])/2
		y2 <- (shad_pol[i+n_shad,2]+shad_pol[i+1+n_shad,2])/2

		if(Debug){
			lines(c(x1,x1),c(y1,y2),lty=2,col="blue")
			lines(c(x2,x2),c(y1,y2),lty=2,col="blue")
			lines(c(x1,x2),c(y1,y1),lty=2,col="blue")
			lines(c(x1,x2),c(y2,y2),lty=2,col="blue")
		}

		bin_pix <- which((x>=x1) & (x<x2))
		in_pix <- bin_pix[which((y[bin_pix]>=y1) & (y[bin_pix]<=y2))]
		out_pix <- bin_pix[which((y[bin_pix]<y1) | (y[bin_pix]>y2))]
		
		if(Debug){
			points(x[in_pix],y[in_pix],col="blue",pch=16)
			points(x[out_pix],y[out_pix],col="red",pch=16)
		}

		n_out <- n_out + length(out_pix)
		n_in <- n_in + length(in_pix)

		shad_area <- shad_area + (x2-x1)*(y2-y1)
	}

	shad_area <- shad_area / prod(ps.pan)
	
	val <- 2.0*n_in - n_out - 2*(shad_area - n_in)
...
eval_shad_v6 <- function(params,x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_shad_pix,x_step){
	...
	tmp <- project_pollock_quantitativeC(h1,h2,R,theta_sat,n)
	x1 <- x0 + tmp*cos(alpha_sat)
	y1 <- y0 + tmp*sin(alpha_sat)

	x <- xy_shad_pix[,1] - x1
	y <- xy_shad_pix[,2] - y1

	#apparent crown in new coordinate system
	x0prime <- x0-x1
	y0prime <- y0-y1
	...
	if(Debug){
		...
		abline(v=0,lty=2)
		rd <- rep(R,Nd)
		xd <- x0prime + rd*cos(phid)
		yd <- y0prime + rd*sin(phid)
		points(x0prime,y0prime,col="green",pch=16)
		lines(xd,yd,col="green",lwd=1)
	}

	shad_pol <- project_pollockC(h1,h2,R,0,0,theta_sun,3*pi/2,n,x_step)
	...
	for(i in 1:(length(xarr)-1)){
		...
		y_max <- (shad_pol[i+n_shad,2]+shad_pol[i+1+n_shad,2])/2

		y_min <- (shad_pol[i,2] + shad_pol[i+1,2])/2
		
		if(x_bin>=x0prime-R && x_bin<=x0prime+R){
			y_apparent <- y0prime + sqrt(R^2-(x_bin-x0prime)^2)
			y_min <- max(c(y_apparent,y_min))
		}

		if(Debug){
			lines(c(x1,x1),c(y_min,y_max),lty=2,col="blue")
			lines(c(x2,x2),c(y_min,y_max),lty=2,col="blue")
			lines(c(x1,x2),c(y_min,y_min),lty=2,col="blue")
			lines(c(x1,x2),c(y_max,y_max),lty=2,col="blue")
		}

		bin_pix <- which((x>=x1) & (x<x2))
		in_pix <- bin_pix[which((y[bin_pix]>=y_min) & (y[bin_pix]<=y_max))]
		if(x_bin<=x0prime-R){
			out_pix <- bin_pix[which((y[bin_pix]<y_min) | (y[bin_pix]>y_max))]
		}else out_pix <- bin_pix[which(y[bin_pix]>y_max)]
		...
		shad_area <- shad_area + (x2-x1)*(y_max-y_min)
	}
...
obj_fun_shadow_v7 <- function(params,x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_pix,pan_pix,x_step){
	...
	if(Debug){
		windows()
		plot(x,y, xlim=1.2*c(-R,R),ylim=c(-5,15),asp=1,pch=16,cex=0.1)
		abline(h=0,lty=2)
		abline(v=0,lty=2)
	}
	...
	for(i in 1:(length(xarr)-1)){
		...
		y2 <- (shad_pol[i+n_shad,2]+shad_pol[i+1+n_shad,2])/2
		y2a <- shad_pol[i+n_shad,2]
		y2b <- shad_pol[i+1+n_shad,2]

		if(Debug){
			lines(c(x1,x1),c(y1a,y2a),lty=2,col="blue")
			lines(c(x2,x2),c(y1b,y2b),lty=2,col="blue")
			lines(c(x1,x2),c(y1a,y1b),lty=2,col="blue")
			lines(c(x1,x2),c(y2a,y2b),lty=2,col="blue")
		}
		...
		#in_pix <- bin_pix[which((y[bin_pix]>=y1) & (y[bin_pix]<=y2))]
		#out_pix <- bin_pix[which((y[bin_pix]<y1) | (y[bin_pix]>y2))]
		in_pix <- bin_pix[which((y_bin>=y1 & y_bin<y1+dy) | (y_bin<=y2 & y_bin>y2-dy))]
		out_pix <- bin_pix[which((y_bin<y1 & y_bin>=y1-dy) | (y_bin>y2 & y_bin<=y2+dy))]
		
		ind_in  <- c(ind_in,in_pix)
		ind_out <- c(ind_out,out_pix)
	...
	val <- mean(pan_pix[ind_out],na.rm=TRUE) - mean(pan_pix[ind_in],na.rm=TRUE)
...
grid_optim_v4 <- function(params,obj_fun,dpar=dpar,lower=lower,upper=upper){
	...
	# these should be passed as function arguments
	x0 <- xy0[1]
	y0 <- xy0[2]
	xy_shad_pix <- xy[shad,,drop=F]
	#obj_fun <- eval_shadC
	...
	for(i1 in 1:length(h_arr)){
		h <- h_arr[i1]
		obj_arr[i1] <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_shad_pix,x_step)
		#obj_arr[i1] <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun, xy_shad_pix, ps.pan)
		#tmp2 <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun, xy_shad_pix, ps.pan)
	}
	...
	if(h==0){
		...
		return(c(h,hf,n,obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_shad_pix,x_step),h_min,h_max,hf_min,hf_max,n_min,n_max))
	}
...
grid_optim_v5 <- function(params,obj_fun,dpar=dpar,lower=lower,upper=upper,xy_pix,pan_pix){
	h  <- params[1]
	hf <- params[2]
	n  <- params[3]
	...
	for(i1 in 1:length(h_arr)){
		h <- h_arr[i1]
		obj_arr[i1] <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_pan,pan_sub,x_step)
		#obj_arr[i1] <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun, xy_shad_pix, ps.pan)
		#tmp2 <- obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun, xy_shad_pix, ps.pan)
	}
	...
	if(h==0){
		...
		n <- 2
		#return(c(h,hf,n,obj_fun(c(h,hf,n),xy0,R,theta_sat,alpha_sat,theta_sun,alpha_sun),h_min,h_max,hf_min,hf_max,n_min,n_max))
		return(c(h,hf,n,obj_fun(c(h,hf,n),x0,y0,R,theta_sat,alpha_sat,theta_sun,alpha_sun,xy_pan,pan_sub,x_step),h_min,h_max,hf_min,hf_max,n_min,n_max))
	}
...
draw_shadow <- function(xy0,R,h,hf,n,theta_sat,alpha_sat,theta_sun,alpha_sun){
	...
	shad_pol <- project_pollockC(h1,h2,R,xy1[1],xy1[2],theta_sun,alpha_sun,n,0.125)
...
pol_to_raster <- function(xy.pol,M,N,xTL,yTL,ps){
	...
	min_col = 1 + round((xmin-xTL)/ps[1]) - 1
	max_col = 1 + round((xmax-xTL)/ps[1]) + 1
	min_row = 1 + round((yTL-ymax)/ps[2]) - 1
	max_row = 1 + round((yTL-ymin)/ps[2]) + 1

	if(max_col<1 || min_col>M || max_row<1 || min_row>N){
		return(numeric(0))
	}
	...
	if(max_col > M) max_col = M;

	xy <- array(0,c(0,2))
	pn_arr <- array(0,0)

	pn_row <- min_col:max_col
	x_arr <- (pn_row-1)*ps[1] + xTL
	nx <- length(x_arr)
	
	for(i_row in min_row:max_row){
		pn_new <- (i_row-1)*M + pn_row
		pn_arr <- c (pn_arr,pn_new)
		y_arr <- yTL - (i_row-1)*ps[2]
		xy <- rbind(xy,cbind(x_arr,rep(y_arr,nx)))
	}

	if(length(pn_arr)>0){
		val <- pn_arr[which(point.in.polygon(xy[,1], xy[,2], xy.pol[,1], xy.pol[,2])>0)]
	} else val <- array(0,0)
